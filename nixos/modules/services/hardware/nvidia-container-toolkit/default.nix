  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf config.hardware.nvidia-container-toolkit.enable {
      ...
      warnings = lib.mkMerge [
        ...
        (lib.mkIf
          (config.virtualisation.docker.enableNvidia && config.virtualisation.docker.rootless.enable)
          [
            "Note that despite setting config.virtualisation.docker.enableNvidia and config.virtualisation.docker.rootless.enable, GPU support will not work for the rootless daemon. Use hardware.nvidia-container-toolkit.enable and CDI instead"
          ]
        )
      ];
      ...
      virtualisation = {
        ...
        docker =
          ...
          in
          {
            ...
            rootless.daemon.settings = lib.mkIf (lib.versionAtLeast dockerVersion "25") {
              features.cdi = true;
            };
          };
      };
